          ...
          <Card key={b.id}>
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => openLogSession(b)} className="text-sm rounded-lg bg-brand-violet text-white px-3 py-1.5 hover:opacity-90">
                  + Log session
                </button>
              </div>
            </div>
            ...
            {b.meet_link && (
              <a href={b.meet_link} target="_blank" rel="noreferrer" className="text-xs text-brand-blue hover:underline">
                Open Meet link
              </a>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3 pt-3 border-t border-gray-100">
              <div>
                <p className="text-xs text-gray-500 mb-1">Sessions logged ({(sessionsByBatch[b.id] || []).length})</p>
                  ...
                  {(sessionsByBatch[b.id] || []).map((s) => (
                    <li key={s.id}>
                      ...
                      {s.recording_link && (
                        <>
                          {' · '}
                          <a href={s.recording_link} target="_blank" rel="noreferrer" className="text-brand-blue hover:underline">Recording</a>
                        </>
                      )}
                    </li>
                  ))}
                  {(sessionsByBatch[b.id] || []).length === 0 && <li className="text-gray-400">No sessions logged yet.</li>}
                </ul>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Your payouts on this batch</p>
                <ul className="text-xs text-gray-600 space-y-1">
                  ...
                  {(payoutsByBatch[b.id] || []).length === 0 && <li className="text-gray-400">No payout recorded yet.</li>}
                </ul>
              </div>
            </div>
          </Card>
        ))}
        {batches.length === 0 && !error && <p className="text-gray-400 text-sm">You're not listed on any batches yet.</p>}
      </div>
      ...
      <Modal open={Boolean(logTarget)} onClose={() => setLogTarget(null)} title="Log a session">
        {logTarget && (
          <form onSubmit={handleLogSession} className="space-y-3">
            <p className="text-sm text-gray-500">{logTarget.name}</p>
            ...
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setLogTarget(null)} className="text-sm text-gray-500 hover:underline">
                Cancel
              </button>
              <button disabled={logging} type="submit" className="text-sm text-brand-green hover:underline disabled:opacity-60">
                {logging ? 'Saving…' : 'Save'}
              </button>
            </div>
          </form>
